...
async function $4e63e094b4590b2b$var$getMediaSource(musicItem, quality) {
    ...
    const API_KEY = ``;
    const fishQualityMap = {
        'low': '128k',
        'standard': '320k',
        'high': 'flac',
        'super': 'flac24bit'
    };
    const targetQuality = fishQualityMap[quality] || '128k';
    const source = 'kw';
    try {
        const requestUrl = `${FISH_API_URL}/url/${source}/${musicItem.id}/${targetQuality}`;
        const response = await (0, ($parcel$interopDefault($M8GQN$axios)))({
            method: 'get',
            url: requestUrl,
            headers: {
                'Content-Type': 'application/json',
                'User-Agent': 'lx-music-script/0.0.3',
        ...
        if (responseBody.code === 0 && responseBody.data) {
            ...
            const errorMessages = {
                1: 'IP被封禁',
                2: '获取音乐URL失败: ' + (responseBody.msg || ''),
                4: '远程服务器错误',
                5: '请求过于频繁，请稍后再试',
                6: '请求参数错误'
            };
            const errorMsg = errorMessages[responseBody.code] || 
                           (responseBody.msg || '未知错误');
            throw new Error(`[${responseBody.code}] ${errorMsg}`);
        }
    } catch (error) {
        console.error('[小蜗音乐-播放修复] 请求失败:', error.message);
        return {
            url: '',
            error: error.message
        };
...
const $4e63e094b4590b2b$var$pluginInstance = {
    ...
    author: "竹佀",
    version: "1",
    srcUrl: "https://raw.gitcode.com/Crystim/mfp/raw/main/%E9%85%B7%E6%88%91Fish.js",
